...
class Dataset:
  ...
  def get_slices(self, data: list[list[str]]) -> list[Tensor]: return [self.combine([self.load_slice(x) for x in fileset]) for fileset in data]
  
  def load_slice(self, path: str) -> Tensor: return Tensor(nib.load(path).get_fdata(dtype=np.float16)[SLICE]).flip(1).transpose().clamp(0, 1)
...
if __name__ == "__main__":
   ...
   dataset = Dataset(val)
   for i in range(len(dataset.labels)): plot_slice(dataset.labels[i])
